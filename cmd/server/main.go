...
func serveScan(w http.ResponseWriter, r *http.Request) {
	http.ServeFile(w, r, "web/static/scan.html")
}
...
func handleScan(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		json.NewEncoder(w).Encode(ScanResponse{Success: false, Error: "invalid request body"})
		return
	}

	if req.Target == "" {
		json.NewEncoder(w).Encode(ScanResponse{Success: false, Error: "target is required"})
		return
	}
	...
	if err != nil {
		json.NewEncoder(w).Encode(ScanResponse{Success: false, Error: fmt.Sprintf("invalid ports: %s", err)})
		return
	}

	ips, err := scanner.ParseIPs(req.Target)
	if err != nil {
		json.NewEncoder(w).Encode(ScanResponse{Success: false, Error: fmt.Sprintf("invalid target: %s", err)})
		return
	}
	...
	json.NewEncoder(w).Encode(ScanResponse{Success: true, Report: &report})
}